#!/usr/bin/env python3
"""Wait for P12 checkpoints and submit one immutable R8 DLC evaluation job."""
...
EXPERIMENT_ID = (
    "FASTWAM-MR-N2-PLACEFOOD-CROSSAGENT-GAUSSIAN-P12-EVAL-R8-DLC-20260818"
)
DISPLAY_NAME = "fastwam-p12-gaussian-eval-r8-dlc-full-20260818"
PROBE_DISPLAY_NAME = "fastwam-p12-gaussian-eval-r8-dlc-graphics-probe-20260818"
WORKSPACE_ID = "270969"
...
SOURCE_ROOT = Path(
    "/cpfs/user/chengjuntao/experiments/FastWAM-p12-eval-r8-dlc-20260818"
)
MODEL_ROOT = Path(
...
TF_OUTPUT = Path(
    "/oss-chengjuntao/artifacts/"
    "fastwam-placefood-crossagent-gaussian-p12-paired-tf-20260818-r8-dlc"
)
STEP500_OUTPUT = Path(
    "/oss-chengjuntao/artifacts/"
    "fastwam-p12-step000500-official-topp-h32-val8-20260818-r8-dlc"
)
STEP1000_OUTPUT = Path(
    "/oss-chengjuntao/artifacts/"
    "fastwam-p12-step001000-official-topp-h32-val8-20260818-r8-dlc"
)
RECORD_ROOT = Path(
    "/oss-chengjuntao/artifacts/"
    "fastwam-placefood-crossagent-gaussian-p12-eval-controller-20260818-r8-dlc"
)
PROBE_RECORD_ROOT = Path(
    "/oss-chengjuntao/artifacts/"
    "fastwam-placefood-crossagent-gaussian-p12-eval-controller-20260818-r8-dlc-graphics-probe"
)
LOCAL_ROOT = Path(
    "/mnt/workspace/experiments/FASTWAM-P12-EVAL-R8-DLC-20260818/runtime"
)
...
def validate_inputs(run_mode: str = "full_eval") -> str:
    ...
    if run_mode == "full_eval":
        ...
        if any(path.exists() or path.is_symlink() for path in outputs()):
            raise RuntimeError("fresh R8-DLC evaluation output already exists")
...
def _validate_expected_projection(
    ...
    """Require every frozen request field while allowing response-only fields."""
    if isinstance(expected, dict):
        if not isinstance(observed, dict):
            raise RuntimeError(f"provider job field changed type: {path}")
        for key, value in expected.items():
            if key not in observed:
                raise RuntimeError(f"provider job omitted frozen field: {path}.{key}")
            _validate_expected_projection(observed[key], value, f"{path}.{key}")
        return
    if isinstance(expected, list):
        if not isinstance(observed, list) or len(observed) != len(expected):
            raise RuntimeError(f"provider job list changed: {path}")
        for index, value in enumerate(expected):
            _validate_expected_projection(observed[index], value, f"{path}[{index}]")
        return
    if observed != expected:
        raise RuntimeError(
            f"provider job changed frozen field: {path}: {observed!r} != {expected!r}"
        )
...
def validate_provider_job(
    ...
    for key, value in expected_settings.items():
        if key == "EnableRDMA":
            ...
            if actual is value:
                continue
            if value is False and actual is True:
                normalizations.append("Settings.EnableRDMA:false_to_true")
                continue
            raise RuntimeError(
                f"provider job changed frozen field: job.Settings.{key}"
            )
        if key not in observed_settings:
            raise RuntimeError(f"provider job omitted frozen field: job.Settings.{key}")
        _validate_expected_projection(
            observed_settings[key], value, f"job.Settings.{key}"
        )

    oversold = observed_settings.get("OversoldType")
    if oversold is not None:
        if oversold != "":
            raise RuntimeError("provider returned non-empty OversoldType")
        normalizations.append("Settings.OversoldType:absent_to_empty")
    if observed.get("SpotStrategy"):
        raise RuntimeError("provider returned spot scheduling")
    for index, spec in enumerate(observed.get("JobSpecs") or []):
        if spec.get("OversoldType") not in (None, ""):
            raise RuntimeError(f"provider returned worker OversoldType at index {index}")
        if spec.get("ElasticSpotSpecs"):
            raise RuntimeError(f"provider returned worker spot scheduling at index {index}")
...
def preflight(run_mode: str) -> tuple[str, dict[str, Any], Any, Any, Any, int]:
    ...
    if duplicates:
        raise RuntimeError(f"refusing duplicate P12 R8 submit: {duplicates}")
...
def main() -> None:
    ...
    if existing:
        raise RuntimeError(f"permanent P12 R8 submission evidence already exists: {existing}")
    if args.audit_only:
        result: dict[str, Any] = {
    ...
    try:
        ...
        raise RuntimeError("another P12 R8 DLC submit supervisor is active") from error
