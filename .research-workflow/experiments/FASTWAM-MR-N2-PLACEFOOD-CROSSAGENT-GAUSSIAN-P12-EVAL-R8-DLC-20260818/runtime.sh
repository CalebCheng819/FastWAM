...
die() {
  printf 'P12_R8_DLC_EVAL_FATAL: %s\n' "$*" >&2
  exit 1
}
...
if [[ "$P12_RUN_MODE" == full_eval ]]; then
...
case "$P12_RUN_MODE" in
  graphics_probe)
    [[ "$gpu_count" == 8 ]] || die "graphics_probe requires the resource-group-supported 1x8 allocation, observed ${gpu_count} GPUs"
    ;;
  full_eval)
    [[ "$gpu_count" == 8 ]] || die "full_eval requires exactly 8 visible GPUs, observed ${gpu_count}"
    ;;
  *)
    die "unknown P12_RUN_MODE: ${P12_RUN_MODE}"
    ;;
esac

scratch_root=$(mktemp -d /tmp/fastwam-p12-r8-dlc.XXXXXXXX)
cleanup() {
  rm -rf -- "$scratch_root"
}
...
mkdir -m 0700 -p -- \
  ...
  "$scratch_root/graphics-lib" "$scratch_root/graphics-manifests" \
  "$scratch_root/graphics-probes" "$P12_RECORD_ROOT/logs"
printf '%s\n' \
  '{"file_format_version":"1.0.0","ICD":{"library_path":"libEGL_nvidia.so.0"}}' \
  >"$scratch_root/graphics-manifests/10_nvidia.json"
ln -sf "$P12_VULKAN_LOADER" "$scratch_root/graphics-lib/libvulkan.so.1"
for library in \
  "$P12_GRAPHICS_ROOT/lib/libEGL.so.1.1.0" \
  "$P12_GRAPHICS_ROOT/driver-lib/libEGL_nvidia.so.570.153.02" \
  "$P12_GRAPHICS_ROOT/driver-lib/libGLX_nvidia.so.570.153.02"; do
  [[ -f "$library" && ! -L "$library" ]] || die "required CPFS EGL/GLX loader is not a regular file: ${library}"
done
ln -sf "$P12_GRAPHICS_ROOT/lib/libEGL.so.1.1.0" "$scratch_root/graphics-lib/libEGL.so.1"
ln -sf "$P12_GRAPHICS_ROOT/lib/libEGL.so.1.1.0" "$scratch_root/graphics-lib/libEGL.so"
ln -sf "$P12_GRAPHICS_ROOT/driver-lib/libEGL_nvidia.so.570.153.02" "$scratch_root/graphics-lib/libEGL_nvidia.so.0"
ln -sf "$P12_GRAPHICS_ROOT/driver-lib/libGLX_nvidia.so.570.153.02" "$scratch_root/graphics-lib/libGLX_nvidia.so.0"
...
apply_cpfs_loader() {
  ...
  prepend_library_paths \
    ...
    "$P12_GRAPHICS_ROOT/driver-lib" \
    "$P12_GRAPHICS_ROOT/lib64" \
    "$P12_GRAPHICS_ROOT/lib"
}

validate_cpfs_egl_loader() {
  "$P12_EVAL_PYTHON" -B - <<'PY'
import ctypes
import ctypes.util

resolved = ctypes.util.find_library("EGL")
if not resolved:
    raise SystemExit("libEGL loader name could not be resolved")
handle = ctypes.CDLL("libEGL.so.1")
if not hasattr(handle, "eglQueryString"):
    raise SystemExit("libEGL.so.1 does not export eglQueryString")
print(f"P12_R8_DLC_CPFS_EGL_LOADER_PASS resolved={resolved}")
PY
}

ensure_sapien_egl_contract() {
  if [[ -z "${__EGL_VENDOR_LIBRARY_FILENAMES:-}" && -z "${__EGL_VENDOR_LIBRARY_DIRS:-}" ]]; then
    export __EGL_VENDOR_LIBRARY_FILENAMES="$scratch_root/graphics-manifests/10_nvidia.json"
  fi
  if [[ -n "${__EGL_VENDOR_LIBRARY_FILENAMES:-}" ]]; then
    [[ -f "$__EGL_VENDOR_LIBRARY_FILENAMES" && ! -L "$__EGL_VENDOR_LIBRARY_FILENAMES" ]] \
      || return 2
  fi
}
...
apply_graphics_profile() {
  ...
  restore_provider_graphics
  case "$profile" in
    provider_explicit_manifest_headless)
      clear_explicit_graphics_selection
      vk_manifest=$(first_regular_file \
        /etc/vulkan/icd.d/nvidia_icd.json \
        /usr/share/vulkan/icd.d/nvidia_icd.json \
        "$P12_GRAPHICS_ROOT/nvidia_icd.json") || return 2
    ...
    cpfs_manifest_headless)
      clear_explicit_graphics_selection
      apply_cpfs_loader
      validate_cpfs_egl_loader || return 2
      test -f "$P12_GRAPHICS_ROOT/nvidia_icd.json" || return 2
      test -f "$P12_GRAPHICS_ROOT/10_nvidia.json" || return 2
      export VK_ICD_FILENAMES="$P12_GRAPHICS_ROOT/nvidia_icd.json"
      export VK_DRIVER_FILES="$P12_GRAPHICS_ROOT/nvidia_icd.json"
      export __EGL_VENDOR_LIBRARY_FILENAMES="$P12_GRAPHICS_ROOT/10_nvidia.json"
      export __GLX_VENDOR_LIBRARY_NAME=nvidia
...
probe_program=$'import os\nfrom pathlib import Path\nfrom diagnose_place_food_fixed import _build_environment\nroot = Path(os.environ["P12_ROBOFACTORY_ROOT"])\nenvironment = _build_environment(root, "PlaceFood-rf")\nenvironment.close()\nprint("P12_R8_DLC_ENVIRONMENT_CONSTRUCTION_PROBE_PASS task=PlaceFood-rf device=0")\n'
profiles=(
  provider_explicit_manifest_headless system_manifest_headless
  provider_native_headless provider_clean_headless system_default_headless
  system_discovered_headless
  system_discovered_sapien_loader cpfs_manifest_headless
)
for profile in "${profiles[@]}"; do
  probe_log="$scratch_root/graphics-probes/${profile}.log"
  if ! apply_graphics_profile "$profile"; then
    printf 'P12_R8_DLC_GRAPHICS_PROFILE_SKIPPED profile=%s reason=unavailable\n' "$profile"
    continue
  fi
  if ! ensure_sapien_egl_contract; then
    printf 'P12_R8_DLC_GRAPHICS_PROFILE_SKIPPED profile=%s reason=invalid-egl-contract\n' "$profile"
    continue
  fi
  set +e
  ...
  if [[ "$probe_rc" == 0 ]]; then
    ...
    printf 'P12_R8_DLC_GRAPHICS_PROFILE_SELECTED profile=%s\n' "$profile"
    cp -- "$probe_log" "$P12_RECORD_ROOT/logs/graphics-${profile}.log"
    break
  fi
  printf 'P12_R8_DLC_GRAPHICS_PROFILE_REJECTED profile=%s rc=%s\n' "$profile" "$probe_rc" >&2
  cp -- "$probe_log" "$P12_RECORD_ROOT/logs/graphics-${profile}.log"
  printf 'P12_R8_DLC_GRAPHICS_PROFILE_LOG_TAIL_BEGIN profile=%s\n' "$profile" >&2
  tail -n 80 -- "$probe_log" >&2 || true
  printf 'P12_R8_DLC_GRAPHICS_PROFILE_LOG_TAIL_END profile=%s\n' "$profile" >&2
done
...
ensure_sapien_egl_contract
if [[ "$P12_RUN_MODE" == graphics_probe ]]; then
  python3 - "$P12_RECORD_ROOT/graphics-probe-summary.json" "$selected_profile" <<'PY'
import json
import os
import sys
from pathlib import Path
...
value = {
    ...
    "status": "SUCCEEDED",
    "graphics_profile": sys.argv[2],
    "claim": "graphics probe only validates PlaceFood-rf environment construction and close",
}
temporary = path.with_name(f".{path.name}.tmp-{os.getpid()}")
temporary.write_text(json.dumps(value, indent=2, sort_keys=True) + "\n")
os.replace(temporary, path)
PY
  ...
  printf 'P12_R8_DLC_GRAPHICS_PROFILE_PROBE_COMPLETE\n'
  exit 0
fi

if [[ "$P12_RUN_MODE" != full_eval ]]; then
  die "unknown run mode after probe path: ${P12_RUN_MODE}"
fi
...
value = {
    "schema_version": "fastwam-p12-r8-dlc-evaluation-summary-v1",
    "status": "COMPLETED",
...
final_state=SUCCEEDED
printf 'P12_R8_DLC_EVAL_SCIENTIFIC_COMPLETE\n'
