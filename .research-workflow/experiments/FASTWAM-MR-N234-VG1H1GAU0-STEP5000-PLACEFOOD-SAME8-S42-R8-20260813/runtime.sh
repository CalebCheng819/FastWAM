...
EXPERIMENT_REL='.research-workflow/experiments/FASTWAM-MR-N234-VG1H1GAU0-STEP5000-PLACEFOOD-SAME8-S42-R8-20260813'
...
required_env=(
  ...
  FASTWAM_EGL_FRONTEND FASTWAM_EGL_FRONTEND_SIZE_BYTES FASTWAM_EGL_DISPATCH
...
scratch_root="$(mktemp -d /tmp/fastwam-gau0-placefood-r8.XXXXXXXX)"
cleanup() {
  rm -rf -- "${scratch_root}"
}
trap cleanup EXIT
...
require_runtime_file() {
  ...
  [[ -f "${path}" && ! -L "${path}" ]] || die "EGL runtime input is not an ordinary non-link file: ${path}"
  [[ "$(stat -c '%s' -- "${path}")" == "${expected_bytes}" ]] || \
    die "EGL runtime input size mismatch: ${path}"
}
...
ln -s -- "${FASTWAM_EGL_FRONTEND}" "${scratch_root}/egl-runtime/libEGL.so.1"
ln -s -- 'libEGL.so.1' "${scratch_root}/egl-runtime/libEGL.so"
export LD_LIBRARY_PATH="${scratch_root}/egl-runtime:${FASTWAM_NVIDIA_GRAPHICS_ROOT}/lib:${FASTWAM_NVIDIA_GRAPHICS_ROOT}/driver-lib${LD_LIBRARY_PATH:+:${LD_LIBRARY_PATH}}"
...
frontend = Path(os.environ["FASTWAM_EGL_FRONTEND"]).resolve(strict=True)
vendor = Path(os.environ["FASTWAM_EGL_VENDOR"]).resolve(strict=True)
shim = Path(os.environ["LD_LIBRARY_PATH"].split(os.pathsep)[0]) / "libEGL.so.1"
if shim.resolve(strict=True) != frontend:
    raise SystemExit(f"EGL frontend shim mismatch: {shim.resolve(strict=True)} != {frontend}")
frontend_handle = ctypes.CDLL(str(shim), mode=ctypes.RTLD_GLOBAL)
...
if not callable(getattr(EGL, "eglQueryString", None)):
    raise SystemExit("PyOpenGL EGL binding lacks eglQueryString")
print(f"GAU0_EGL_RUNTIME_PREFLIGHT_PASS frontend={frontend} vendor={vendor}")
'
