...
EXPERIMENT_REL='.research-workflow/experiments/FASTWAM-MR-N234-VG1H1GAU0-STEP5000-PLACEFOOD-SAME8-S42-R20-20260815'

die() {
  printf 'GAU0_R20_EVAL_FATAL: %s\n' "$*" >&2
  exit 1
}
...
scratch_root="$(mktemp -d /tmp/fastwam-gau0-placefood-r20.XXXXXXXX)"
cleanup() {
  rm -rf -- "${scratch_root}"
}
trap cleanup EXIT
...
for profile in "${profiles[@]}"; do
  ...
  if ! apply_graphics_profile "${profile}"; then
    printf 'GAU0_R20_GRAPHICS_PROFILE_SKIPPED profile=%s reason=unavailable\n' "${profile}"
    continue
  fi
  set +e
  ...
  if [[ "${probe_rc}" == 0 ]]; then
    ...
    printf 'GAU0_R20_GRAPHICS_PROFILE_SELECTED profile=%s\n' "${profile}"
    rm -- "${probe_log}"
    break
  fi
  printf 'GAU0_R20_GRAPHICS_PROFILE_REJECTED profile=%s rc=%s\n' "${profile}" "${probe_rc}" >&2
...
printf 'GAU0_R20_EVAL_SCIENTIFIC_COMPLETE\n'
