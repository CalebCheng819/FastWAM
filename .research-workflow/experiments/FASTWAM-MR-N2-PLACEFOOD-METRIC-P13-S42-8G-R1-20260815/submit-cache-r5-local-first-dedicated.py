#!/usr/bin/env python3
"""Audit or exactly-once submit the dedicated-quota P13 R5 cache job."""
...
DISPLAY_NAME = "fastwam-p13-metric-cache-s42-8g-r5-local-first-dedicated-20260817"
RUN_ID = DISPLAY_NAME
OUTPUT_ROOT = (
    "/oss-chengjuntao/artifacts/"
    "fastwam-placefood-metric-geometry-60x80-s42-v1-"
    "r5-local-first-dedicated-20260817"
)
SOURCE_REVISION = "36417a7bb3860955d9a18a3a38e3430b52faf9bc"
SOURCE_BUNDLE = (
    "/oss-chengjuntao/artifacts/fastwam-p13-runtime-20260815/"
    "FastWAM-p13-local-publish-36417a7.bundle"
)
WORKER_COMMAND = (
    "exec /bin/bash /oss-chengjuntao/artifacts/fastwam-p13-runtime-20260817/"
    "cache-worker-r5-local-first-dedicated.sh"
)
...
def validate_request_map(body: dict[str, Any]) -> None:
    ...
    if "OversoldType" in settings or "OversoldType" in specs[0]:
        raise RuntimeError("oversold scheduling is forbidden for R5")
    if body.get("SpotStrategy") or specs[0].get("ElasticSpotSpecs"):
        raise RuntimeError("spot scheduling is forbidden for R5")
    if settings.get("EnableRDMA") is not False or settings.get("AllocateAllRDMADevices") is not False:
        raise RuntimeError("cache job must not allocate RDMA")
    if (settings.get("Tags") or {}).get("scheduler") != "dedicated-quota":
    ...
    if {key: envs.get(key) for key in expected_envs} != expected_envs:
        raise RuntimeError("run, output, or source binding mismatch")
    if body.get("UserCommand") != WORKER_COMMAND:
        raise RuntimeError("R5 worker command mismatch")
...
def preflight(body: dict[str, Any], client: Any, models: Any) -> tuple[Any, int]:
    ...
    if output.exists() or output.is_symlink():
        raise RuntimeError(f"R5 versioned output already exists: {output}")
    jobs = list_jobs(client, models)
    duplicates = duplicate_jobs(jobs)
    if duplicates:
        raise RuntimeError(f"duplicate P13 R5 launch target: {duplicates}")
...
def main() -> None:
    ...
    base = {
        "schema": "fastwam-p13-dlc-submit-r5-v1",
