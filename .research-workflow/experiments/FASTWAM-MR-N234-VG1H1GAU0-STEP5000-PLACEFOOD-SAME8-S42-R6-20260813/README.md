...
R6 is a new, isolated identity after R5 passed the dependency and checkpoint
preflights but failed before the first evaluation episode with `pinned Python resolved target changed`.
The shared CPFS Python entry has the same immediate
symlink text on the control node and worker, while its final realpath is
container-local. R6 therefore freezes the shared entry, immediate symlink text,
Python 3.12 version, implementation cache tag, and SOABI; it deliberately does
not freeze a container-private final realpath.
...
The frozen source import binding introduced for R3 is retained. R6 freezes the
...
in submit, and again before any evaluator process starts. The R1, R2, R3, R4, and R5 latches remain preserved and are neither retried nor reused.
