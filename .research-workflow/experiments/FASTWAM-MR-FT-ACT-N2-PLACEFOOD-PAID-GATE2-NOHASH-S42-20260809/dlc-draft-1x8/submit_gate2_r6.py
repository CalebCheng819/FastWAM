#!/usr/bin/env python3
"""Independent R6 identity after the terminal, permanently retired R5 attempt."""
...
if not stat.S_ISREG(_controller_info.st_mode) or _CONTROLLER_PATH.is_symlink():
    raise RuntimeError("R6 controller must be an exact sibling regular file")
_controller_spec = importlib.util.spec_from_file_location(
    "fastwam_gate2_r6_controller", _CONTROLLER_PATH
)
if _controller_spec is None or _controller_spec.loader is None:
    raise RuntimeError("unable to create the exact R6 controller import spec")
...
controller.EXPERIMENT_ID = (
    "FASTWAM-MR-FT-ACT-N2-PLACEFOOD-PAID-GATE2-NOHASH-R6-S42-20260811"
)
controller.SUBMISSION_TAG_PREFIX = "fastwam-gate2-nohash-r6-s42"
controller.DISPLAY_NAME_PREFIX = "fw-g2-nh-r6-s42"
controller.CONTROL_ENTRYPOINT = "submit_from_ssh970_r6.sh"
controller.SOURCE_PREFIX = Path(
    "/oss-chengjuntao/artifacts/fastwam-nohash-source-snapshots"
)
# R6 is accepted only from the newly published source-r5 snapshot.  The R5
# attempt, source-r4 tree, ledger, tag, and output identity are all retired.
