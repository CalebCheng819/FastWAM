#!/usr/bin/env python3
"""Independent R8 identity after retiring incomplete source-r6/source-r7."""
...
if not stat.S_ISREG(_controller_info.st_mode) or _CONTROLLER_PATH.is_symlink():
    raise RuntimeError("R8 controller must be an exact sibling regular file")
_controller_spec = importlib.util.spec_from_file_location(
    "fastwam_gate2_r8_controller", _CONTROLLER_PATH
)
if _controller_spec is None or _controller_spec.loader is None:
    raise RuntimeError("unable to create the exact R8 controller import spec")
...
controller.EXPERIMENT_ID = (
    "FASTWAM-MR-FT-ACT-N2-PLACEFOOD-PAID-GATE2-NOHASH-R8-S42-20260811"
)
controller.SUBMISSION_TAG_PREFIX = "fastwam-gate2-nohash-r8-s42"
controller.DISPLAY_NAME_PREFIX = "fw-g2-nh-r8-s42"
controller.CONTROL_ENTRYPOINT = "submit_from_ssh970_r8.sh"
controller.SOURCE_PREFIX = Path(
    "/oss-chengjuntao/artifacts/fastwam-nohash-source-snapshots"
)
# The interrupted source-r6 and fail-closed source-r7 publications are retired.
# R8 can bind only this new, fully verified publication from the reviewed tree.
