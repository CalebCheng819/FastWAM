#!/usr/bin/env python3
"""Independent R5 identity after the terminal, permanently retired R4 attempt."""
...
if not stat.S_ISREG(_controller_info.st_mode) or _CONTROLLER_PATH.is_symlink():
    raise RuntimeError("R5 controller must be an exact sibling regular file")
_controller_spec = importlib.util.spec_from_file_location(
    "fastwam_gate2_r5_controller", _CONTROLLER_PATH
)
if _controller_spec is None or _controller_spec.loader is None:
    raise RuntimeError("unable to create the exact R5 controller import spec")
...
controller.SUBMISSION_TAG_PREFIX = "fastwam-gate2-nohash-r5-s42"
controller.DISPLAY_NAME_PREFIX = "fw-g2-nh-r5-s42"
controller.CONTROL_ENTRYPOINT = "submit_from_ssh970_r5.sh"
controller.SOURCE_PREFIX = Path(
    "/oss-chengjuntao/artifacts/fastwam-nohash-source-snapshots"
)
# R5 prepare is accepted only from the newly published source-r4 snapshot.
# No R4 attempt, ledger, tag, output root, or source snapshot may be reused.
