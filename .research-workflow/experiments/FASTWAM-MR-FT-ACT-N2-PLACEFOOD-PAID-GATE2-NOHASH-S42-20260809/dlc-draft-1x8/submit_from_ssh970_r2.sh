...
# R1 and R2 share this admission lock even though their durable ledgers differ.
...
exec 9>"${LOCK_FILE}"
if ! flock -n 9; then
  echo "Error: another SSH970 Gate2-R2 controller holds the single-writer lock." >&2
  exit 75
fi

export FASTWAM_CONTROL_NODE=ssh970
export FASTWAM_LOCK_FD=9
exec "${CONTROL_PYTHON}" "${SCRIPT_DIR}/submit_gate2_r2.py" "$@"
