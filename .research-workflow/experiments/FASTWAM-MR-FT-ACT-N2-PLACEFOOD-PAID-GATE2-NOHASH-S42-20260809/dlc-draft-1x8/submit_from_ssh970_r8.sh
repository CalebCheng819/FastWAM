...
# This lock is only the SSH970 local collision guard.  The new R8 experiment's
# permanent OSS O_EXCL latch remains the cross-node single-mutation boundary.
LOCK_FILE="${LOCK_DIR}/.gate2-nohash-r8-submit.lock"
...
exec 9>"${LOCK_FILE}"
if ! flock -n 9; then
  echo "Error: another SSH970 Gate2-R8 controller holds the single-writer lock." >&2
  exit 75
fi
