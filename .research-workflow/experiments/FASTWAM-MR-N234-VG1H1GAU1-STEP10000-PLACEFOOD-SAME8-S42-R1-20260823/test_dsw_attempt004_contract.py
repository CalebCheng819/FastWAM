...
class DswAttempt007ContractTests(unittest.TestCase):
    def test_runtime_pins_four_gpu_two_wave_same8_contract(self) -> None:
        ...
        self.assertIn("STEP10K_DSW_EVAL_GL_GATE=PASS", runtime)
        self.assertIn("import cv2", runtime)
        self.assertIn("import mani_skill", runtime)
        self.assertIn("import sapien", runtime)
        self.assertIn("import tasks.place_food", runtime)
        self.assertIn("import utils.scenes", runtime)
        self.assertIn("from OpenGL import EGL", runtime)
        self.assertIn('LD_LIBRARY_PATH="/usr/lib/x86_64-linux-gnu:${FASTWAM_NVIDIA_GRAPHICS_ROOT}/lib:', runtime)
        ...
        self.assertLess(runtime.index("STEP10K_DSW_EVAL_GL_GATE=PASS"), runtime.index('mkdir -m 0700 -- "${FASTWAM_CONTROL_ROOT}"'))
    ...
    def test_dsw_aggregator_changes_only_run_identity(self) -> None:
        spec = importlib.util.spec_from_file_location("attempt006_aggregator", AGGREGATOR)
