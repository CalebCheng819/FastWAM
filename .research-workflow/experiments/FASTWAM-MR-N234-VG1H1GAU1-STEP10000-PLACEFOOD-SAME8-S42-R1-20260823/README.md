...
- `attempt-004` / DSW 4-GPU: maps the public `metadata_no_hash` contract to the model loader's strict `stat_cmp` mode, first runs panel episode 0 as a complete single-episode gate, then evaluates the unchanged SAME8 panel in two waves (`0..3`, `4..7`). The formal output is published only after all eight terminal shards pass the original frozen aggregator.
