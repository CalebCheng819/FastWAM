#!/usr/bin/env python3
"""Wait for P12 checkpoints and submit one immutable R7 DLC evaluation job."""
...
EXPERIMENT_ID = (
    "FASTWAM-MR-N2-PLACEFOOD-CROSSAGENT-GAUSSIAN-P12-EVAL-R7-DLC-20260817"
)
DISPLAY_NAME = "fastwam-p12-gaussian-eval-r7-dlc-full-20260817"
PROBE_DISPLAY_NAME = "fastwam-p12-gaussian-eval-r7-dlc-graphics-probe-20260817"
WORKSPACE_ID = "270969"
...
SOURCE_ROOT = Path(
    "/cpfs/user/chengjuntao/experiments/FastWAM-p12-eval-r7-dlc-20260817"
)
MODEL_ROOT = Path(
...
TF_OUTPUT = Path(
    "/oss-chengjuntao/artifacts/"
    "fastwam-placefood-crossagent-gaussian-p12-paired-tf-20260817-r7-dlc"
)
STEP500_OUTPUT = Path(
    "/oss-chengjuntao/artifacts/"
    "fastwam-p12-step000500-official-topp-h32-val8-20260817-r7-dlc"
)
STEP1000_OUTPUT = Path(
    "/oss-chengjuntao/artifacts/"
    "fastwam-p12-step001000-official-topp-h32-val8-20260817-r7-dlc"
)
RECORD_ROOT = Path(
    "/oss-chengjuntao/artifacts/"
    "fastwam-placefood-crossagent-gaussian-p12-eval-controller-20260817-r7-dlc"
)
PROBE_RECORD_ROOT = Path(
    "/oss-chengjuntao/artifacts/"
    "fastwam-placefood-crossagent-gaussian-p12-eval-controller-20260817-r7-dlc-graphics-probe"
)
LOCAL_ROOT = Path(
    "/mnt/workspace/experiments/FASTWAM-P12-EVAL-R7-DLC-20260817/runtime"
)
...
def validate_inputs(run_mode: str = "full_eval") -> str:
    ...
    if run_mode == "full_eval":
        ...
        if any(path.exists() or path.is_symlink() for path in outputs()):
            raise RuntimeError("fresh R7-DLC evaluation output already exists")
...
def preflight(run_mode: str) -> tuple[str, dict[str, Any], Any, Any, Any, int]:
    ...
    if duplicates:
        raise RuntimeError(f"refusing duplicate P12 R7 submit: {duplicates}")
...
def main() -> None:
    ...
    if existing:
        raise RuntimeError(f"permanent P12 R7 submission evidence already exists: {existing}")
    if args.audit_only:
        result: dict[str, Any] = {
    ...
    try:
        ...
        raise RuntimeError("another P12 R7 DLC submit supervisor is active") from error
    ...
    if observed.get("DisplayName") != target_display or observed.get("Priority") != 7:
        raise RuntimeError("submitted P12 R7 job identity mismatch")
    receipt = {
        **response_record,
