...
- Graphics: select the first runtime profile that can construct and close a
  real PlaceFood-rf environment. R7 materializes the missing generic EGL and
  NVIDIA EGL/GLX loader names in a private directory and verifies that
  `libEGL.so.1` exports `eglQueryString` before the environment probe. CPU
  rendering fallback is forbidden.
- Outputs: fresh OSS paths ending in `r7-dlc`; existing R2/R3/R5 evidence is
...
8-GPU full evaluation is rejected until the same R7 probe record contains both a `SUCCEEDED`
