...
die() {
  printf 'P12_R7_DLC_EVAL_FATAL: %s\n' "$*" >&2
  exit 1
}
...
scratch_root=$(mktemp -d /tmp/fastwam-p12-r7-dlc.XXXXXXXX)
cleanup() {
  rm -rf -- "$scratch_root"
}
...
print(f"P12_R7_DLC_CPFS_EGL_LOADER_PASS resolved={resolved}")
PY
}
...
for profile in "${profiles[@]}"; do
  ...
  if ! apply_graphics_profile "$profile"; then
    printf 'P12_R7_DLC_GRAPHICS_PROFILE_SKIPPED profile=%s reason=unavailable\n' "$profile"
    continue
  fi
  set +e
  ...
  if [[ "$probe_rc" == 0 ]]; then
    ...
    printf 'P12_R7_DLC_GRAPHICS_PROFILE_SELECTED profile=%s\n' "$profile"
    cp -- "$probe_log" "$P12_RECORD_ROOT/logs/graphics-${profile}.log"
    break
  fi
  printf 'P12_R7_DLC_GRAPHICS_PROFILE_REJECTED profile=%s rc=%s\n' "$profile" "$probe_rc" >&2
  cp -- "$probe_log" "$P12_RECORD_ROOT/logs/graphics-${profile}.log"
done
...
PY
  ...
  printf 'P12_R7_DLC_GRAPHICS_PROFILE_PROBE_COMPLETE\n'
  exit 0
fi
...
value = {
    "schema_version": "fastwam-p12-r7-dlc-evaluation-summary-v1",
    "status": "COMPLETED",
...
final_state=SUCCEEDED
printf 'P12_R7_DLC_EVAL_SCIENTIFIC_COMPLETE\n'
