...
die() {
  printf 'P12_R5_DLC_EVAL_FATAL: %s\n' "$*" >&2
  exit 1
}
...
case "$P12_RUN_MODE" in
  graphics_probe)
    [[ "$gpu_count" -ge 1 ]] || die "graphics_probe mode requires at least one visible GPU, observed ${gpu_count}"
    ;;
  full_eval)
...
scratch_root=$(mktemp -d /tmp/fastwam-p12-r5-dlc.XXXXXXXX)
cleanup() {
  rm -rf -- "$scratch_root"
}
...
for profile in "${profiles[@]}"; do
  ...
  if ! apply_graphics_profile "$profile"; then
    printf 'P12_R5_DLC_GRAPHICS_PROFILE_SKIPPED profile=%s reason=unavailable\n' "$profile"
    continue
  fi
  set +e
  ...
  if [[ "$probe_rc" == 0 ]]; then
    ...
    printf 'P12_R5_DLC_GRAPHICS_PROFILE_SELECTED profile=%s\n' "$profile"
    cp -- "$probe_log" "$P12_RECORD_ROOT/logs/graphics-${profile}.log"
    break
  fi
  printf 'P12_R5_DLC_GRAPHICS_PROFILE_REJECTED profile=%s rc=%s\n' "$profile" "$probe_rc" >&2
  cp -- "$probe_log" "$P12_RECORD_ROOT/logs/graphics-${profile}.log"
done
...
PY
  ...
  printf 'P12_R5_DLC_GRAPHICS_PROFILE_PROBE_COMPLETE\n'
  exit 0
fi
...
value = {
    "schema_version": "fastwam-p12-r5-dlc-evaluation-summary-v1",
    "status": "COMPLETED",
...
final_state=SUCCEEDED
printf 'P12_R5_DLC_EVAL_SCIENTIFIC_COMPLETE\n'
