...
DISPLAY_NAME = "fastwam-p12-gaussian-eval-r5-dlc-full-20260816"
PROBE_DISPLAY_NAME = "fastwam-p12-gaussian-eval-r5-dlc-graphics-probe-20260816"
WORKSPACE_ID = "270969"
...
SOURCE_ROOT = Path(
    "/cpfs/user/chengjuntao/experiments/FastWAM-p12-eval-r5-dlc-20260816"
)
MODEL_ROOT = Path(
...
TF_OUTPUT = Path(
    "/oss-chengjuntao/artifacts/"
    "fastwam-placefood-crossagent-gaussian-p12-paired-tf-20260816-r5-dlc"
)
STEP500_OUTPUT = Path(
    "/oss-chengjuntao/artifacts/"
    "fastwam-p12-step000500-official-topp-h32-val8-20260816-r5-dlc"
)
STEP1000_OUTPUT = Path(
    "/oss-chengjuntao/artifacts/"
    "fastwam-p12-step001000-official-topp-h32-val8-20260816-r5-dlc"
)
RECORD_ROOT = Path(
    "/oss-chengjuntao/artifacts/"
    "fastwam-placefood-crossagent-gaussian-p12-eval-controller-20260816-r5-dlc"
)
PROBE_RECORD_ROOT = Path(
    "/oss-chengjuntao/artifacts/"
    "fastwam-placefood-crossagent-gaussian-p12-eval-controller-20260816-r5-dlc-graphics-probe"
)
LOCAL_ROOT = Path(
    "/mnt/workspace/experiments/FASTWAM-P12-EVAL-R5-DLC-20260816/runtime"
)
...
def validate_inputs(run_mode: str = "full_eval") -> str:
    ...
    if run_mode == "full_eval":
        ...
        if any(path.exists() or path.is_symlink() for path in outputs()):
            raise RuntimeError("fresh R5-DLC evaluation output already exists")
...
def validate_request_map(body: dict[str, Any], run_mode: str) -> None:
    ...
    expected_gpu = "1" if run_mode == "graphics_probe" else "8"
    expected_topology = "1x1" if run_mode == "graphics_probe" else "1x8"
...
def preflight(run_mode: str) -> tuple[str, dict[str, Any], Any, Any, Any, int]:
    ...
    if duplicates:
        raise RuntimeError(f"refusing duplicate P12 R5 submit: {duplicates}")
...
def main() -> None:
    ...
    if existing:
        raise RuntimeError(f"permanent P12 R5 submission evidence already exists: {existing}")
    if args.audit_only:
        result: dict[str, Any] = {
    ...
    try:
        ...
        raise RuntimeError("another P12 R5 DLC submit supervisor is active") from error
    ...
    if observed.get("DisplayName") != target_display or observed.get("Priority") != 7:
        raise RuntimeError("submitted P12 R5 job identity mismatch")
    receipt = {
        **response_record,
        ...
        "requested_topology": "1x1" if run_mode == "graphics_probe" else "1x8",
