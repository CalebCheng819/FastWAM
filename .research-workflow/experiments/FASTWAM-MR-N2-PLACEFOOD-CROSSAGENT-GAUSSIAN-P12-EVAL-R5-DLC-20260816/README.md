...
- 1-GPU graphics probe mode: validate Vulkan/EGL + PlaceFood-rf construction
  before any full 8-GPU eval submission.
...
- Outputs: fresh OSS paths ending in `r5-dlc`; existing R2/R3 evidence is
...
already exist. Submit the 1-GPU graphics probe first. The 8-GPU full evaluation
is rejected until the same R5 probe record contains both a `SUCCEEDED`
