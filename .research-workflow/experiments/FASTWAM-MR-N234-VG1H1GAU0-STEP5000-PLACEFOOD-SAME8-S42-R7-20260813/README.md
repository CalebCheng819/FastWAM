...
R7 is a new, isolated identity after R6 passed dependency and checkpoint
preflights but failed before the first evaluation episode because its frozen
worker Python ABI was 3.12 while the DLC image provides Python 3.10. R7 freezes
the stable CPFS Python 3.10 entry, its immediate CPFS symlink target, Python
3.10.20 version, implementation cache tag, and SOABI. Its dedicated dependency
layer contains the exact packages required by the evaluator import chain.
...
The frozen source import binding introduced for R3 is retained. R7 freezes the
...
in submit, and again before any evaluator process starts. The R1 through R6
