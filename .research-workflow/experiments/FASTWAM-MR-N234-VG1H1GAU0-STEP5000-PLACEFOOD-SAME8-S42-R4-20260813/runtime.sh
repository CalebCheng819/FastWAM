...
EXPERIMENT_REL='.research-workflow/experiments/FASTWAM-MR-N234-VG1H1GAU0-STEP5000-PLACEFOOD-SAME8-S42-R4-20260813'
...
required_env=(
  ...
  FASTWAM_PYTHON_TARGET FASTWAM_PYTHON_RESOLVED_TARGET FASTWAM_BASELINE_ROOT FASTWAM_TRAINING_SOURCE_COMMIT
...
export PYTHONPATH="${FASTWAM_SOURCE_ROOT}/src${PYTHONPATH:+:${PYTHONPATH}}"
...
pythonpath_head = Path(os.environ["PYTHONPATH"].split(os.pathsep, 1)[0]).resolve(strict=True)
expected_runtime = (expected_src / "fastwam" / "runtime.py").resolve(strict=True)
actual_runtime = Path(runtime.__file__).resolve(strict=True)
if pythonpath_head != expected_src:
    raise SystemExit(f"frozen src is not first on PYTHONPATH: {pythonpath_head} != {expected_src}")
if actual_runtime != expected_runtime:
    raise SystemExit(f"wrong fastwam.runtime imported: {actual_runtime} != {expected_runtime}")
...
print(f"GAU0_FROZEN_RUNTIME_IMPORT_PASS path={actual_runtime}")
'

scratch_root="$(mktemp -d /tmp/fastwam-gau0-placefood-r4.XXXXXXXX)"
cleanup() {
  rm -rf -- "${scratch_root}"
}
trap cleanup EXIT
