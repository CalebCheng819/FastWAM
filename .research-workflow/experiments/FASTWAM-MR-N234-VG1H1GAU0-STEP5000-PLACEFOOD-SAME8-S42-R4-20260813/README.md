...
R4 is a new, isolated identity after R3 correctly imported the frozen source
but failed before the first evaluation episode. The archived GAU0 checkpoint
uses the original native-v2 full-checkpoint envelope and predates both the
`state_kind` field and `action_attention_topology` metadata. R4 accepts that
...
The frozen `${FASTWAM_SOURCE_ROOT}/src` import binding introduced for R3 is
retained. The R1, R2, and R3 latches remain preserved and are neither retried
nor reused.
