...
R3 is a new, isolated identity after R2 reached the worker but imported an
unrelated live CPFS checkout because the source snapshot root, rather than its
`src` directory, was prepended to `PYTHONPATH`. R3 prepends the frozen
`${FASTWAM_SOURCE_ROOT}/src` path and fails closed unless `fastwam.runtime`
resolves exactly to that snapshot and exposes `create_multi_robot_fastwam`.
The R1 and R2 latches remain preserved and are neither retried nor reused.
