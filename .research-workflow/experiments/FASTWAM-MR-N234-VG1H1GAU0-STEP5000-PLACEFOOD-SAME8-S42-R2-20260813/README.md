...
R2 is a new, isolated identity after R1 was rejected before job creation by the
DLC API because `Settings.Tags` used the SDK-accepted list shape. R2 freezes the
server-compatible string map instead. The R1 latch remains preserved and is not
retried or reused.
