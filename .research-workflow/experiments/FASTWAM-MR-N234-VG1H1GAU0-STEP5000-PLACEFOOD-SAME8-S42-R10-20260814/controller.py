...
EXPERIMENT_ID = "FASTWAM-MR-N234-VG1H1GAU0-STEP5000-PLACEFOOD-SAME8-S42-R10-20260814"
RUN_ID = "fastwam-gau0-placefood-same8-r10-20260814"
DISPLAY_NAME = "fw-gau0-placefood-same8-r10"
WORKSPACE_ID = "270969"
RESOURCE_ID = "quotaksvqq2oh2pg"
SOURCE_ROOT = Path("/oss-chengjuntao/artifacts/fastwam-nohash-source-snapshots/fastwam-gau0-placefood-same8-eval-20260814-r19")
OUTPUT_ROOT = Path("/oss-chengjuntao/artifacts/fastwam-gau0-placefood-same8-eval-20260814-r10")
DURABLE_ROOT = Path("/oss-chengjuntao/artifacts/fastwam-gau0-placefood-same8-eval-20260814-r10-controller")
...
LOCAL_ROOT = Path("/run/fastwam-dlc-submit-state/workspace-270969/gau0-placefood-same8-r10")
...
'''
    with tempfile.TemporaryDirectory(prefix="fastwam-gau0-glvnd-r10-") as temporary:
...
def validate_terminal() -> dict[str, Any]:
    ...
    expected_files = {
        f"{arm}/shard{shard}-episodes{2 * shard}-{2 * shard + 1}/{name}"
        for arm in module.ARMS
        for shard in range(4)
        for name in ("episodes.jsonl", "run_manifest.json", "summary.json")
    }
    ...
    expected_dirs = {"."} | {arm for arm in module.ARMS} | {
        f"{arm}/shard{shard}-episodes{2 * shard}-{2 * shard + 1}"
        for arm in module.ARMS
        for shard in range(4)
    }
    files, directories, total_bytes = tree_inventory(OUTPUT_ROOT)
    if files != expected_files or directories != expected_dirs or len(files) != 29 or len(directories) != 11:
    ...
    if (
        terminal.get("gaussian_conditioning") is not False
        or terminal.get("episode_invocations") != 16
        or terminal.get("evaluator_processes") != 8
    ):
