...
def validate_arm(temp_root: Path, arm: str) -> dict[str, Any]:
    ...
    for shard_index in range(4):
        ...
        shard = temp_root / arm / f"shard{shard_index}-episodes{start}-{end}"
        names = exact_directory_names(shard)
        if names != ["episodes.jsonl", "run_manifest.json", "summary.json"]:
            raise RuntimeError(f"unexpected shard allowlist for {arm}/{shard_index}: {names}")
        records = load_jsonl(shard / "episodes.jsonl")
        if len(records) != 2:
            raise RuntimeError(f"{arm}/{shard_index} has {len(records)} episode records")
        summary = load_json(shard / "summary.json")
        manifest = load_json(shard / "run_manifest.json")
        for offset, episode in enumerate(records):
            index = start + offset
            expected = {
                "task_index": index,
                "panel_index": index,
            ...
            for key, value in expected.items():
                if episode.get(key) != value:
                    raise RuntimeError(f"{arm}/{shard_index}/{index} {key} mismatch: {episode.get(key)!r}")
            if episode.get("task_name") != "PlaceFood-rf":
                raise RuntimeError(f"{arm}/{shard_index}/{index} task mismatch")
            episodes.append(episode)
        if summary.get("status") != "PASS" or summary.get("episodes_completed") != 2:
            raise RuntimeError(f"{arm}/{shard_index} evaluator summary is not PASS")
        if summary.get("infrastructure_errors") != 0 or summary.get("episodes_requested") != 2:
            raise RuntimeError(f"{arm}/{shard_index} evaluator accounting mismatch")
        argv = manifest.get("argv")
        if (
            not isinstance(argv, list)
            or "--no-gaussian-conditioning" not in argv
            or "--num-episodes" not in argv
            or argv[argv.index("--num-episodes") + 1] != "2"
        ):
            raise RuntimeError(f"{arm}/{shard_index} does not prove two-episode GAU0 execution")
