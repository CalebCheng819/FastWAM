...
EXPERIMENT_REL="${FASTWAM_EXPERIMENT_REL_OVERRIDE:-.research-workflow/experiments/FASTWAM-MR-N234-VG1H1GAU0-STEP5000-PLACEFOOD-SAME8-S42-R17-20260814}"
...
scratch_root="$(mktemp -d "${FASTWAM_SCRATCH_TEMPLATE_OVERRIDE:-/tmp/fastwam-gau0-placefood-r17.XXXXXXXX}")"
cleanup() {
  rm -rf -- "${scratch_root}"
}
trap cleanup EXIT
...
mkdir -m 0700 -- "${scratch_root}/xdg-cache" "${scratch_root}/xdg-runtime" \
  ...
  "${scratch_root}/pycache" "${scratch_root}/egl-runtime"

[[ -f "${FASTWAM_EGL_FRONTEND}" ]] || die "frozen EGL frontend is not a regular file"
[[ ! -L "${FASTWAM_EGL_FRONTEND}" ]] || die "frozen EGL frontend must not be a symlink"
...
export FASTWAM_GL_SHIM_ROOT="${scratch_root}/egl-runtime"
export LD_LIBRARY_PATH="${FASTWAM_GL_SHIM_ROOT}:${FASTWAM_NVIDIA_GRAPHICS_ROOT}/driver-lib:/usr/local/cuda-12.8/lib64${LD_LIBRARY_PATH:+:${LD_LIBRARY_PATH}}"
...
print(f"GAU0_NARROW_EGL_RUNTIME_PREFLIGHT_PASS loader={loader_prefix} modules={modules}")
'
