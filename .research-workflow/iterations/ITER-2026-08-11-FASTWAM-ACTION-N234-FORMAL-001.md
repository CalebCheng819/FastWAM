...
- 源快照修正：首次发布的 `fastwam-action-n234-formal-20260811-r1` 在控制端执行帮助命令时被 Python 自动写入一个 `__pycache__` 文件，因此立即退役且不用于 prepare/submit。Gate2 与 formal SSH970 入口均改为显式禁用字节码写入，并增加静态回归测试；下一份唯一源快照使用 `fastwam-action-n234-formal-20260811-r2`，旧 r1 不删除、不覆盖。
