...
# Formal 2-worker x 8-GPU launcher for the joint-safe Table11 VG0/H1/GAU1 rerun.
...
TASK_PROFILE="robofactory_table11_vg0_hub1_gau1_scratch50k_224_1e-4"
SCALE_PROFILE="robofactory_multi_robot_16gpu_scratch50k"
...
is_uint "${MACHINE_RANK:-x}" || \
  die "RANK must be an integer in [0,$((EXPECTED_MACHINES - 1))]"
...
overrides = [
    ...
    "+scale=robofactory_multi_robot_16gpu_scratch50k",
...
if resolved["model"]["training_mode"] != "action_only_cache":
    raise SystemExit("model training mode is not action_only_cache")
...
if resolved["model"]["loss"]["lambda_video"] != 0.0:
    raise SystemExit("video loss must remain disabled")
if resolved["model"]["loss"]["lambda_action"] != 1.0:
    raise SystemExit("action loss must remain enabled at weight 1")
for split in ("train", "val"):
    data = resolved["data"][split]
    if data["load_future_video"] is not False:
        raise SystemExit(f"{split} future-video loading must remain disabled")
...
if run_mode == "preflight-one-step":
    ...
    print(
        f"table11 safe config gate: world={expected_world} global_batch={expected_world} "
        "updates=50000 save_every=1000 rolling_keep_last=2"
    )
