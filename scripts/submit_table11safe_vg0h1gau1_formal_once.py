#!/usr/bin/env python3
"""Audit and submit the frozen joint-safe Table11 VG0/H1/GAU1 2x8 request once."""
...
RUN_ID = "fastwam-table11safe-vg0h1gau1-scratch50k-s42-16g-r1-20260831"
ATTEMPT_ID = "attempt-r1-20260831"
WORKSPACE_ID = "270969"
...
PRELAUNCH = LAUNCH_ROOT / "prelaunch-formal-2x8-r1.json"
AUDIT_RECORD = LAUNCH_ROOT / "submission-formal-2x8-r1-audit.json"
RECEIPT = LAUNCH_ROOT / "submission-formal-2x8-r1-receipt.json"
...
REAL_DATA_PREFLIGHT_RUN_ID = (
    "fastwam-table11safe-vg0h1gau1-scratch-preflight-s42-8g-r1-20260831"
)
REAL_DATA_PREFLIGHT_ATTEMPT_ID = "attempt-r1-20260831"
REAL_DATA_PREFLIGHT_OUTPUT_DIR = (
    f"/oss-chengjuntao/artifacts/{REAL_DATA_PREFLIGHT_RUN_ID}"
)
BUNDLE = str(
    LAUNCH_ROOT / "fastwam-table11safe-vg0h1gau1-scratch50k-source-r1-20260831.bundle"
)
...
EXPECTED_SETTINGS = {
    ...
    "Tags": {
        "experiment": "TABLE11SAFE-VG0H1GAU1-SCRATCH50K",
        "gaussian": "enabled",
...
def validate_embedded_launcher(
    ...
    body: dict,
    *,
    expected_machines: int = 2,
    expected_world: int = 16,
    bundle: str = BUNDLE,
    commit: str = COMMIT,
    ...
    with tempfile.TemporaryDirectory(prefix="table11-submit-audit.") as temp:
        ...
        expected = git_stdout(
            ...
            str(checkout),
            "show",
            f"{commit}:scripts/launch_table11safe_vg0h1gau1_2x8_dlc.sh",
        )
...
def validate_document(document: dict) -> dict:
    ...
    require(
        ...
        == {
            "bundle": BUNDLE,
            "code_commit": COMMIT,
            "path": "scripts/launch_table11safe_vg0h1gau1_2x8_dlc.sh",
        },
    ...
    require(
        body["Description"]
        == (
            "Joint-safe RoboFactory table11 VG0H1GAU1 action-only no-video "
            "scratch-from-generic-base training: "
            "optimizer steps 0 to 50000, 50000 fresh-optimizer updates, "
            "2 workers x 8 GPUs, world-16 global batch"
        ),
        "Description drift",
    )
    require(
...
def main() -> int:
    ...
    require(
        COMMIT != UNPUBLISHED_COMMIT,
        "published commit is not frozen; audit/submit forbidden",
    )
