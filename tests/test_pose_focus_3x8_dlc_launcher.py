...
P11_TASK_NAME = "robofactory_placefood_spatial_transport_p11_224_5e-6.yaml"
...
class PoseFocusLauncherTests(unittest.TestCase):
    def fixture(self, root: Path) -> dict[str, str]:
        ...
        (repo / "configs" / "task" / P11_TASK_NAME).write_bytes(
            (REPO / "configs" / "task" / P11_TASK_NAME).read_bytes()
        )
    ...
    def test_dry_run_resolves_p11_spatial_transport_contract(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            env = self.fixture(Path(directory))
            env["FASTWAM_POSE_FOCUS_TASK_PROFILE"] = P11_TASK_NAME.removesuffix(".yaml")
            result = subprocess.run(
                ["bash", str(LAUNCHER)],
                cwd=REPO,
                env=env,
            ...
            self.assertEqual(result.returncode, 0, result.stderr)
            self.assertIn(
                "task=robofactory_placefood_spatial_transport_p11_224_5e-6",
                result.stdout,
            )
    ...
    def test_renderer_selects_p11_source_and_objective(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            output = root / "job.json"
            bundle, commit, _ = self.committed_launcher_bundle(root)
            command = [
                ...
                str(RENDERER),
                "--run-id",
                "fastwam-placefood-spatial-transport-p11-test",
                "--attempt-id",
                "attempt-1",
                "--output",
                str(output),
                ...
                commit,
                "--task-profile",
                P11_TASK_NAME.removesuffix(".yaml"),
                "--source-weight",
                P6_SOURCE_WEIGHT,
                "--allow-local-bundle-for-tests",
            ]
            ...
            self.assertEqual(
                request["Settings"]["Tags"]["objective"],
                "placefood-spatial-transport-causal-sampling",
            )
            self.assertEqual(
                request["Settings"]["Tags"]["initialization"],
                "P6-action-step1000-weights-only",
