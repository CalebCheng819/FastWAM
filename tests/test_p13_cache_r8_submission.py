...
SUBMITTER = EXPERIMENT / "submit-cache-r8-complete-glvnd-dedicated.py"
REQUEST = EXPERIMENT / "cache-create-job-r8-complete-glvnd-dedicated.json"
WORKER = EXPERIMENT / "cache-worker-r8-complete-glvnd-dedicated.sh"

SPEC = importlib.util.spec_from_file_location("p13_cache_r8", SUBMITTER)
MODULE = importlib.util.module_from_spec(SPEC)
assert SPEC.loader is not None
SPEC.loader.exec_module(MODULE)


class P13CacheR8SubmissionTest(unittest.TestCase):
    def test_request_is_priority_seven_dedicated_and_complete_glvnd_first(self):
        ...
        self.assertEqual(
            ...
            "complete-glvnd-abi-before-sensitive-import",
        )

    def test_worker_binds_gpu_zero_and_r8_runtime(self):
        source = WORKER.read_text(encoding="utf-8")
        subprocess.run(["bash", "-n", str(WORKER)], check=True)
        self.assertIn("export CUDA_VISIBLE_DEVICES=0", source)
        self.assertIn("fastwam-p13-runtime-20260817-r8-complete-glvnd", source)
